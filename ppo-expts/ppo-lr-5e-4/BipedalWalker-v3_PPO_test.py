...
class Environment(Process):
    def __init__(self, env_idx, child_conn, env_name, state_size, action_size, visualize=False):
        ...
        # if self.record:
        #     print("here=========================================\n")
        #     self.env = gym.make(env_name, hardcore = hardcore, render_mode='human')
        #     self.env = gym.wrappers.RecordVideo(self.env, video_folder=f'{outfolder}/video', episode_trigger = lambda x: x % 50 == 0) #Saving every n = 1 episode
        # else:
        #     self.env = gym.make(env_name, hardcore = hardcore, render_mode='rgb_array')#'human')
    ...
    def run(self):
        ...
        while True:
            ...
            self.child_conn.send([state, reward, terminated, truncated , info])
...
class PPOAgent:
    ...
    def __init__(self, env_name, model_name=""):
        ...
        if self.record:
            print("here=========================================\n")
            self.env = gym.make(env_name, hardcore = hardcore, render_mode='rgb_array')
            # self.env = gym.wrappers.RecordVideo(self.env, video_folder=f'{outfolder}', episode_trigger = lambda x: x % 50 == 0) #Saving every n = 1 episode
    ...
    def run_multiprocesses(self, num_worker = 4):
        ...
        while self.episode < self.EPISODES:
            ...
            for worker_id, parent_conn in enumerate(parent_conns):
                next_state, reward, terminated, truncated , info  = parent_conn.recv()
                done = truncated or terminated 
