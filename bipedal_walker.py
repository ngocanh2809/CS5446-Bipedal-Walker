...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def step(self, action: np.ndarray):
        ...
        shaping = (
            130 * pos[0] / SCALE
        )  # REWARD DIST_TRAVEL: moving forward is a way to receive reward (normalized to get 300 on completion)
        shaping -= 5.0 * abs(
            state[0]
        )  # REWARD HULL_ANGLE: keep head straight, other than that and falling, any behavior is unpunished

        reward = 0
        if self.prev_shaping is not None:
            reward = shaping - self.prev_shaping
        self.prev_shaping = shaping
        # REWARD: Difference between this step and previous step

        for a in action:
            reward -= 0.00035 * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1)
            # REWARD PUNISH_FOR_MOVEMENT:normalized to about -50.0 using heuristic, more optimal agent should spend less

        terminated = False
        if self.game_over or pos[0] < 0:
            reward = -100
            terminated = True
            # REWARD MOVE_FORWARD_OR_DIE
